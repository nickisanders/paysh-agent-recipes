...
| [🖼️ NFT Floor Watcher](./nft-floor-watcher) | Watches a Solana NFT collection's floor and alerts when it crosses your buy or sell line, once per crossing, not every check | pay.sh market data · state machine · cron | _soon_ |
...
| [👁️ Vision Extractor](./vision-extractor) | An image in, typed JSON out: sends a photo (receipt, invoice, screenshot) to Gemini via pay.sh and pulls the fields you name | pay.sh vision (Gemini) · image → JSON | _soon_ |
